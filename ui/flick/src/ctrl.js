...
var chessground = require('flickground');
...
module.exports = function(opts) {
  ...
  var onUserFlick = function(vel, key, win) {
    if (this.data.game.turns+1 - this.data.game.startedAtTurn > 1)
      (this.clock || this.correspondenceClock).update(this.clock.data.white, this.clock.data.black);
    this.data.game.player = (this.data.game.player === 'white') ? 'black' : 'white';
    this.data.game.turns++;
    this.sendFlick(vel, key, win, this.data.player.color);
  }.bind(this);
  ...
  this.sendFlick = function(vel, key, win, color) {
    var flick = {
      vel: vel,
      key: key,
      win: win,
  ...
  this.flick = function(o) {      
      if (this.data.player.color !== o.color && this.chessground.data.turnColor === o.color) {
          this.chessground.discreteTest({piece: o.key, vec: o.vel});
	  this.chessground.data.turnColor = o.color === "white" ? "black" : "white";
	  if (this.data.game.turns+1 - this.data.game.startedAtTurn === 2)
	      (this.clock || this.correspondenceClock).update(this.clock.data.white, this.clock.data.black);
	  this.data.game.player = (this.data.game.player === 'white') ? 'black' : 'white';
	  this.data.game.turns++;
	  if (o.clock)(this.clock || this.correspondenceClock).update(o.clock.white, o.clock.black);
